...
struct dispVals* createDispVals(struct movie* list) {
...
struct dispVals* createDispValsList(struct movie* list) {
	// Create first node in list, increment movie pointer to next node
	struct dispVals* head = createDispVals(list);
	...
	while (movieList) {
		...
		if (movieList->year == tail->year) {
			...
			struct dispVals* newNode = createDispVals(movieList);
			tail->next = newNode;
		}
...
int main(int argc, char *argv[])
{
    ...
    while (exit == 0)
    {
        ...
        {
            struct dispVals* temp = createDispValsList(list);
            printKeysValue(temp);
            freeKeysValue(temp);
            break;
        }
